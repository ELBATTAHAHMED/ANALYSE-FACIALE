...
def get_employee_image(connection, emp_id):
    try:
        ...
        cursor.execute(sql, (emp_id,))
        image_data = cursor.fetchone()[0]  # Récupérer les données binaires de l'image depuis la base de données
        cursor.close()
        return image_data  # Retourne directement les données binaires de l'image
    except Error as e:
